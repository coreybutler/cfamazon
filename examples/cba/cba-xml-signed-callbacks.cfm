<cfscript>
	...
	cart.setCalculateTaxRates(true);
	cart.setCalculatePromotions(true);
	cart.setCalculateShippingRates(true);
	//cart.setprocessOrderOnCallbackFailure(true);
...
<html>
	<head>
		...
		<title>XML Signed Cart Callback-enabled Example</title>
	</head>
	<body>
		<cfset xml = XmlParse(cart.getXML())/>
		
		<em>Custom Order (Callbacks)</em>
